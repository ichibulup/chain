...
export const bulkUpdateStaffScheduleStatus = async (req: Request, res: Response) => {
  try {
    ...
    if (!isAdminRole(actor.role)) {
      ...
      const restaurantIds = new Set(schedules.map((item: { restaurantId: string }) => item.restaurantId));
      ...
      try {
        await requireManagerOrAdminForRestaurant(userId, restaurantId as string);
      } catch (error) {
        const message = error instanceof Error ? error.message : 'Forbidden';
        const status = message === 'Restaurant not found' ? 404 : 403;
        return res.status(status).json({
