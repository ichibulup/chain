...
/** Middleware chính: xác thực Supabase access token, attach req.auth & req.user, kiểm tra quyền nếu truyền options */
export function withAuth(options?: WithAuthOptions) {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      const supabase = await createServer(req, res);
      ...
      const user = await syncFromSupabase(data.user);
      ...
      if (options?.role?.length) {
        const currentRole = resolveUserRole({
          dbUser: user ? { role: user.role as UserRole } : undefined,
          authUser: data.user,
        });
