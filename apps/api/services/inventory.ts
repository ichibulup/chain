...
import { InventoryItemShortly, OrganizationShortly, RestaurantShortly, SupplierShortly, WarehouseShortly } from "@/lib/interfaces";
import {
  getIssueTransactionType,
  normalizeBalanceDate,
  computeItemPricing,
  generateIssueCode,
  generateReceiptCode,
} from '@/lib/utils/generaters';
import {
  checkWarehouseReceiptExists,
  checkWarehouseIssueExists,
} from '@/services/helper';

type DbClient = PrismaClient | Prisma.TransactionClient;

const generateUniqueWarehouseNumber = async (
  generateCode: () => string,
...
export const generateUniqueReceiptNumber = async (
  ...
  return generateUniqueWarehouseNumber(
    generateReceiptCode,
    (candidate) => checkWarehouseReceiptExists(candidate),
    'Failed to generate unique receipt number',
    maxAttempts,
  );
};

export const generateUniqueIssueNumber = async (
  maxAttempts = 5,
) => {
  return generateUniqueWarehouseNumber(
    generateIssueCode,
    (candidate) => checkWarehouseIssueExists(candidate),
...
export const createWarehouseReceipt = async (data: CreateWarehouseReceipt) => {
  try {
    ...
    const warehouseReceipt = await Database.$transaction(async (tx) => {
      const receiptNumber = data.receiptNumber || await generateUniqueReceiptNumber();
...
export const createWarehouseIssue = async (data: CreateWarehouseIssue) => {
  try {
    ...
    const warehouseIssue = await Database.$transaction(async (tx) => {
      const issueNumber = data.issueNumber || await generateUniqueIssueNumber();
