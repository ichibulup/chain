...
export async function createUser(
  ...
  try {
    ...
    if (existingUser) {
      ...
      const updatedUser = await Database.$transaction(async (tx: Prisma.TransactionClient) => {
        ...
        const nextRole =
          existingMembership && rolePriority[existingMembership.role as OrganizationRole] > rolePriority[membershipRole]
      ...
      try {
        const { organization, restaurant, supplier } = await getUserOrgRestaurant(updatedUser.id);
    ...
    try {
      const result = await Database.$transaction(async (tx: Prisma.TransactionClient) => {
    ...
    try {
      const { organization, restaurant, supplier } = await getUserOrgRestaurant(createdUser.id);
...
async function syncUserDbToAuthByLookupId(lookupId: string) {
  ...
  const { organization, restaurant, supplier } = await getUserOrgRestaurant(existingDbUser.id);
