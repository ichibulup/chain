...
import { InventoryItem, Warehouse, WarehouseIssue, WarehouseReceipt } from "@/models/inventory";
...
export const checkWarehouseReceiptExists = async (receiptNumber?: string): Promise<boolean> => {
  try {
    ...
    console.error('Error checking warehouse receipt existence:', error);
    return false;
  }
};

export const checkWarehouseIssueExists = async (issueNumber?: string): Promise<boolean> => {
  try {
    ...
    console.error('Error checking warehouse issue existence:', error);
    return false;
  }
};
